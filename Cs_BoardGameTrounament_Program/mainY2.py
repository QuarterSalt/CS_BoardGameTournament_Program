...
class LeaderboardApp:
    def __init__(self, root):
        ...
        # Load data
        self.players_points_info_Y2 = self.load_data()
        self.players_dict_info_Y2 = {
            "NumberOfPlayers": 0,
            "TotalGlobalPoints": 0
        }
    ...
    def load_data(self):
        try:
            with open('players_points_info_Y2.json', 'r') as f:
    ...
    def save_data(self):
        with open('players_points_info_Y2.json', 'w') as f:
            json.dump(self.players_points_info_Y2, f)
    ...
    def update_leaderboard(self):
        ...
        players_without_stats = {name: score for name, score in self.players_points_info_Y2.items()
        ...
        # Update global stats
        self.players_points_info_Y2["TotalGlobalPoints"] = total_points
        self.players_points_info_Y2["NumberOfPlayers"] = num_players
    ...
    def calculate_points(self):
        try:
            ...
            # Calculate points
            if not self.players_points_info_Y2:
                average_points = 1
                current_points = 1
            else:
                average_points = sum(self.players_points_info_Y2.values()) / len(self.players_points_info_Y2)
                current_points = self.players_points_info_Y2.get(player_name, 1)
            ...
            # Update points
            if player_name not in self.players_points_info_Y2:
                self.players_points_info_Y2[player_name] = 0
            self.players_points_info_Y2[player_name] += awarded_points
    ...
    def perform_action(self):
        ...
        if not player or player not in self.players_points_info_Y2:
            messagebox.showerror("Error", "Player not found")
            return

        if action == "delete":
            del self.players_points_info_Y2[player]
            messagebox.showinfo("Success", f"{player} deleted")
        elif action == "remove":
            try:
                points = int(self.points_to_remove.get())
                self.players_points_info_Y2[player] = max(0, self.players_points_info_Y2[player] - points)
